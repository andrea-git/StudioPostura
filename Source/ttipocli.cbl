      ...
      *{TOTEM}PRGID
       PROGRAM-ID.          ttipocli.
       AUTHOR.              Andrea.
       DATE-WRITTEN.        sabato 4 maggio 2019 14:37:25.
       REMARKS.
      *{TOTEM}END
      ...
      * Properties & User defined Working Stoarge
       ...
       77 Form1-Tb-1-Handle
                  USAGE IS HANDLE OF WINDOW.
       78 titolo VALUE IS "Studio - Tabella tipologie Clienti". 
       77 E-ESCI           PIC  9
                  VALUE IS 1.
       77 E-NUOVO          PIC  9
                  VALUE IS 0.
      ...
      ***********************************************************
       ...
       77 TMP-Form1-tipocli-RESTOREBUF  PIC X(615).
      ...
      * FILE'S LOCK MODE FLAG
       ...
       77 DataSet1-tipocli-KEY1-ORDER  PIC X VALUE "A".
          88 DataSet1-tipocli-KEY1-Asc  VALUE "A".
          88 DataSet1-tipocli-KEY1-Desc VALUE "D".

       77 tipocli-tcl-k-descrizione-SPLITBUF  PIC X(57).

       01 OLD-tcl-rec.
           05 OLD-tcl-chiave.
               10 OLD-tcl-codice       PIC  x(3).
           05 OLD-tcl-dati.
               10 OLD-tcl-descrizione  PIC  x(50).
               10 OLD-tcl-sconto     PIC  9(3)v99.
               10 OLD-tcl-vuoti.
                   15 FILLER           PIC  9(18).
                   15 FILLER           PIC  9(18).
                   15 FILLER           PIC  9(18).
      ...
      * <TOTEM:EPT. INIT:ttipocli, INIT:ttipocli, BeforeDeclarative>
       codtcl-ERR SECTION.
           ...
           when "35"
                display message "File [tipocli] not found!"
                           title titolo
                            icon 3
                set errori to true
           when "39"
      ...
      *    After Open
       ...
       OPEN-tipocli.
      * <TOTEM:EPT. INIT:ttipocli, FD:tipocli, BeforeOpen>
      * <TOTEM:END>
           OPEN  I-O tipocli
           IF STATUS-tipocli = "35"
              OPEN OUTPUT tipocli
                IF Valid-STATUS-tipocli
                   CLOSE tipocli
                   OPEN I-O tipocli
                END-IF
           END-IF
           ...
           END-IF
      * <TOTEM:EPT. INIT:ttipocli, FD:tipocli, AfterOpen>
      * <TOTEM:END>
           .
      ...
      *    After Close
       ...
       CLOSE-tipocli.
      * <TOTEM:EPT. INIT:ttipocli, FD:tipocli, BeforeClose>
      * <TOTEM:END>
           CLOSE tipocli
           .
       ...
       DataSet1-Change-CurrentKey-Asc.
           EVALUATE DataSet1-KEYIS
           WHEN 1
              MOVE "A" TO DataSet1-tipocli-KEY1-ORDER
           END-EVALUATE
           .
       ...
       DataSet1-Change-CurrentKey-Desc.
           EVALUATE DataSet1-KEYIS
           WHEN 1
              MOVE "D" TO DataSet1-tipocli-KEY1-ORDER
           END-EVALUATE
           .
      ...
      * GRID
       ...
       Form1-DataSet1-Update-Key.
           MOVE tcl-rec OF tipocli TO
                     Form1-MULKEY-TMPBUF
           PERFORM Form1-CLEAR
           PERFORM Form1-INIT-DATA
           MOVE Form1-MULKEY-TMPBUF TO
                     tcl-rec OF tipocli
           PERFORM DataSet1-tipocli-Read
       ...
       Form1-First.
           ...
           PERFORM DataSet1-tipocli-INITSTART
           PERFORM DataSet1-tipocli-START
           IF NOT Valid-STATUS-tipocli
              PERFORM Form1-Extended-File-Status
              EXIT PARAGRAPH
           END-IF
      ...
      * <TOTEM:END>
           PERFORM DataSet1-tipocli-Read-Next
           IF NOT Valid-STATUS-tipocli
              PERFORM Form1-Extended-File-Status
              EXIT PARAGRAPH
           END-IF
      ...
      * <TOTEM:END>
       ...
       Form1-Previous.
              PERFORM Form1-Buf-To-Fld
              PERFORM DataSet1-tipocli-START-LESS
           IF NOT Valid-STATUS-tipocli
              PERFORM Form1-Extended-File-Status
              EXIT PARAGRAPH
           END-IF
      ...
      * <TOTEM:END>
           PERFORM DataSet1-tipocli-Read-Prev
           IF NOT Valid-STATUS-tipocli
              PERFORM Form1-Extended-File-Status
              EXIT PARAGRAPH
           END-IF
      ...
      * <TOTEM:END>
       ...
       Form1-Next.
              PERFORM Form1-Buf-To-Fld
              PERFORM DataSet1-tipocli-START-GREATER
           IF NOT Valid-STATUS-tipocli
              PERFORM Form1-Extended-File-Status
              EXIT PARAGRAPH
           END-IF
      ...
      * <TOTEM:END>
       ...
       Form1-Last.
           ...
           PERFORM DataSet1-tipocli-INITEND
           PERFORM DataSet1-tipocli-START-NOTGREATER
           IF NOT Valid-STATUS-tipocli
              PERFORM Form1-Extended-File-Status
              EXIT PARAGRAPH
           END-IF
      ...
      * <TOTEM:END>
      * write
           PERFORM DataSet1-tipocli-Rec-Rewrite
           IF Valid-STATUS-tipocli
      ...
      * delete
           PERFORM Form1-Buf-To-Fld
           PERFORM DataSet1-tipocli-Rec-Delete
           IF Valid-STATUS-tipocli
              PERFORM Form1-CLEAR
              PERFORM Form1-DUMMY-DELETE-INIT
              MOVE "303" TO TOTEM-MSG-ID
              MOVE "00" to STATUS-tipocli
           END-IF
      ...
      * <TOTEM:END>
       ...
       Form1-Save-Status.
           MOVE DataSet1-tipocli-KEY1-ORDER TO TMP-Form1-KEY1-ORDER
           MOVE DataSet1-KEYIS TO TMP-Form1-KEYIS
           MOVE tcl-rec OF tipocli TO 
              TMP-Form1-tipocli-RESTOREBUF
           .             

       Form1-Restore-Status.
           MOVE TMP-Form1-KEY1-ORDER TO DataSet1-tipocli-KEY1-ORDER
           MOVE TMP-Form1-KEYIS TO DataSet1-KEYIS
           MOVE TMP-Form1-tipocli-RESTOREBUF TO
              tcl-rec OF tipocli
           PERFORM DataSet1-tipocli-START
           IF Valid-STATUS-tipocli
              PERFORM DataSet1-tipocli-Read-Next
           ELSE
              PERFORM DataSet1-tipocli-INITREC
           END-IF
           PERFORM UNTIL NOT Valid-STATUS-tipocli OR
              (Valid-STATUS-tipocli AND
                 tcl-rec OF tipocli = 
                   TMP-Form1-tipocli-RESTOREBUF)
              PERFORM DataSet1-tipocli-Read-Next
           END-PERFORM
      ...
      *--->fine.                 
           ...
           if scelta = mb-yes
              ...
              delete tipocli record invalid continue end-delete
      ...
      * <TOTEM:PARA. INTESTAZIONE>
           ...
           modify form1-gd-1(1, 3), cell-data = "Sconto" 
           .
      * <TOTEM:END>
      ...
      * <TOTEM:PARA. LOAD-RECORD>
           ...
           start tipocli key is >= tcl-chiave
                  invalid continue
              not invalid
                  perform varying riga from 2 by 1
                     ...
                     read tipocli next
                          at end exit perform
                      not at end
                          move tcl-codice      to col-codice
                          move tcl-descrizione to col-des
                          move tcl-sconto    to col-sconto
      ...
      *--->modifica (isacco)
           if nuovo 
              ...
              if mod = 1
                 close tipocli
                 open i-o tipocli allowing readers
              else
                 set tutto-ok to true
                 if NoSalvato
                    move 1 to mod
      ...
      * <TOTEM:PARA. VALORE-RIGA>
           ...
           inquire form1-gd-1(riga, 3), cell-data tcl-sconto  
           .
      * <TOTEM:END>
      ...
      * <TOTEM:PARA. VALORE-OLD-RIGA>
           if vecchio
              inquire form1-gd-1(riga, 1) cell-data old-tcl-codice
      ...
      * <TOTEM:PARA. form1-gd-1-Ev-Msg-Finish-Entry>
           ...
           inquire form1-gd-1(riga, 3), cell-data col-sconto.
           modify  form1-gd-1(riga, 3), cell-data col-sconto.
      ...
      * <TOTEM:PARA. form1-gd-1-Ev-Msg-Begin-Entry>
           ...
           if colonna = 1
              if tcl-codice not = zero and tcl-codice not = spaces
                 set event-action to event-action-fail
              end-if
           else 
              if mod = 0
